...
# -----------------------------------------------------------------------------
# File locations (looks next to the app file, then one level up / repo root)
# -----------------------------------------------------------------------------
APP_DIR = Path(__file__).resolve().parent

def first_existing(*paths: Path) -> Path:
    """Return the first path that exists; else the first candidate (loader will error if missing)."""
    for p in paths:
        if p.exists():
            return p
    return paths[0]
...
# CSVs for each tab
DISTRICT_CSV = first_existing(APP_DIR / "dashboard_wfp_template.csv",
                              APP_DIR.parent / "dashboard_wfp_template.csv")

NATE_CSV     = first_existing(APP_DIR / "dashboard_wfp_nate.csv",
                              APP_DIR.parent / "dashboard_wfp_nate.csv")

# -----------------------------------------------------------------------------
# Helpers
# -----------------------------------------------------------------------------
# District tiers (Tab 1 only)
TIER1 = {2, 5, 7, 8}
...
def tier_from_district(d):
    try:
        ...
        return "Unassigned"
    if d in TIER1: return "Tier 1"
    if d in TIER2: return "Tier 2"
    return "Unassigned"
...
def normalize_headers(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df.columns = (
        df.columns.astype(str)
          .str.strip()
          .str.replace("\uFEFF", "", regex=True)  # strip BOM if present
          .str.lower()
          .str.replace(" ", "_")
    )
    alias = {
...
def support_bar(frame: pd.DataFrame, index_label: str):
    ...
    support_melt = frame.melt(id_vars=[index_label], value_vars=["1","2","3","4","5"],
                              var_name="SID", value_name="count").fillna(0)
    pivot = support_melt.pivot_table(index=index_label, columns="SID",
...
def load_districts_csv(path: Path) -> pd.DataFrame:
    if not path.exists():
        raise FileNotFoundError(
            f"District CSV not found at '{path}'. Commit 'dashboard_wfp_template.csv' "
            "next to the app or at repo root."
        )
    df = pd.read_csv(path)
    ...
    df["tier_label"] = df["district"].apply(tier_from_district)  # derive tier from district
...
with tab_districts:
    ...
    if f.empty:
        ...
        st.download_button("Download District Summary (CSV)",
